...
function 再生終了(){
    console.log("再生終了");
    beep.play();
    let 次講 = document.querySelector(".active-lecture").closest(".lecture-container").nextElementSibling;
    if(次講 != null){
        次講.querySelector("a").click();//現在再生中の親親要素の隣のaタグをクリックして再生開始
    }else{
        let 次回 = document.querySelector(".active-section").closest(".section-container").nextElementSibling;
        次回.querySelector("p").click();//次の単元を選択
        setTimeout(()=>{document.querySelector(".lecture").click();},1000);//1秒後先頭のレクチャーを再生開始

    }

}
function 記述貼り付け(){
    var 入力枠 = document.querySelector("#tou-exam-report-textarea");//レポート入力エリア
    if (入力枠 != null){//存在するならば
...
function ページ移動判定(){
    if(document.querySelector(".loading-outer").style.display != "none"){//画面が読み込み待ちの場合
        ...
        if (new RegExp("^#/courses/[0-9]{4}/sections/[0-9]{5}/lectures/[0-9]{6}/materials/[0-9]{6}$").test(location.hash)){//記述式ページ
            ...
            document.querySelector("div.indent.mt25").querySelectorAll("input").forEach((x)=>{x.onchange = ()=>{//全てのinput要素に対し、変更された時
                ...
                switch(document.querySelector("input").type){//要素のタイプで変更
                    ...
                    case "radio":
                        ...
                        break;
                    case "text":
                        document.querySelector("div.indent.mt25").querySelectorAll("input").forEach((y)=>{処理アレイ.push(y.value)});
                        break;
                    default:
                ...
                if(回答[対象ID] != null){
                    if (JSON.parse(回答[対象ID]).length > 0) {
                        JSON.parse(回答[対象ID]).forEach((keys)=>{
                            document.querySelector("div.indent.mt25").querySelectorAll("input").forEach((x)=>{
                                switch(x.type){
                                    case "checkbox":
            ...
            toumsg("小テスト正誤確認...");
            回答 = JSON.parse(localStorage.getItem("授業回答情報"));
            document.querySelectorAll(".quiz-result__marks").forEach((child)=>{
            ...
            ロゴ書き換え(true);
            document.querySelectorAll(".course-progress.card").forEach((e)=>{
                try{
...
function ロゴ書き換え(トップフラグ){
    let ロゴ = document.querySelector(".top-logo")
    if (トップフラグ == true){
...
function 進捗率取得(){
    document.querySelector("li.course-summary").children.item(1).href = "/#/";//ばつ印で授業から抜けてトップに移動するリンクを設置
    document.querySelectorAll("p.professor__sub").forEach((e)=>{
